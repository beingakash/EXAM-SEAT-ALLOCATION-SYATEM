...
include_once("connect.php");
 ...
 if( isset($_GET['edit']))
     {
             $id = $_GET['edit'];
             $res = mysqli_query($connect,"SELECT * FROM dept WHERE ID='$id'");
     ...
     {
              $id = $_POST['id'];
               $UpdateQuery =  mysqli_query($connect,"UPDATE dept SET deptid ='$_POST[did]', name='$_POST[name]' WHERE ID='$id'");
               if(mysql_query($UpdateQuery)){
               die("strange error");
            }
            else{
                //echo '<script language="javascript">';
                //echo 'alert("Successful Updated");location.href="dept.php"';
                //echo '</script>';
              echo '
<!DOCTYPE html>
<html>
...
<div class="row container">
  ...
  <div class="col-xs-6 space">
    ...
    <form class="myform" action="deptedit.php" method="post">
      <div class="form-group">
      <label for="exampleInputDid">Dept ID</label>
      <input type="text" class="form-control" required="required" name="did" value="<?php echo $row[1]; ?>">
    </div>
    <div class="form-group">
    <label for="exampleInputDid">Name</label>
    <input type="text" class="form-control" required="required" name="name" value="<?php echo $row[2]; ?>">
  </div>
  <input type="hidden" name="id" value="<?php echo $row[0]; ?>">
  <input class="btn btn-primary" type="submit" value="Update" name="submit">
  <a class="btn btn-default" href="dept.php" role="button">Cancel</a>
</form>
  </div>
</div>
