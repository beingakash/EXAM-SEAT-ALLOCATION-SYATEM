...
include_once("connect.php");
 ...
 if( isset($_GET['edit']))
     {
             $id = $_GET['edit'];
             $res = mysqli_query($connect,"SELECT * FROM faculty WHERE ID='$id'");
     ...
     if( isset($_POST['submit']))
     {
              ...
              $passwordmd5 = md5($_POST['pass']);
               $UpdateQuery = mysqli_query($connect,"UPDATE faculty SET tid ='$_POST[tid]', fname='$_POST[name]', lname='$_POST[lname]', department='$_POST[dept]' , gender='$_POST[gender]', dob='$_POST[dob]',doj='$_POST[doj]', email='$_POST[email]' , password='$passwordmd5' WHERE ID='$id'");
	...
	<div class="navbar-header navbar-right">
		<ul class="nav navbar-nav">
          ...
          <ul class="dropdown-menu">
            ...
            <li><a href="hall"><span class="glyphicon glyphicon-home" aria-hidden="true"> Hall</a></li>
          </ul>
        </li>
...
<div class="row container">
  ...
  <div class="col-xs-6 space">
    ...
    <form class="myform" action="feditstd.php" method="post">
      <div class="form-group">
      <label for="exampleInputDob">Std ID</label>
      <input type="text" class="form-control" required="required" name="tid" value="<?php echo $row[1];   ?>">
      </div>
      <div class="form-group">
      ...
      <input type="text" class="form-control" name="dept" required="required" value="<?php echo $row[4];   ?>" readonly>
      </div>
      <div class="form-group">
      ...
      <div class="form-group">
      <label for="exampleInputDob">Doj</label>
      <input type="date" class="form-control" name="doj" required="required" value="<?php echo $row[7];   ?>">
      </div>
      <div class="form-group">
      <label for="exampleInputDob">Email</label>
      <input type="email" class="form-control" name="email" required="required" value="<?php echo $row[8];   ?>">
      </div>
      <div class="form-group">
      <label for="exampleInputDob">Password</label>
      <input type="password" class="form-control" name="pass" required="required" value="<?php echo $row[9];   ?>">
      </div>
  <input type="hidden" name="id" value="<?php echo $row[0]; ?>">
<input class="btn btn-primary" type="submit" value="Update" name="submit">
<a class="btn btn-default" href="faculty.php" role="button">Cancel</a>
</form>
<br><br>
  </div>
</div>
